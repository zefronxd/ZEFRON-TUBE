...
const DETECT_TTL_MS = 24 * 60 * 60 * 1000;

export function LanguageSelector({ className }: { className?: string }) {
  const languagePref = useZefron((s) => s.languagePref);
  const setLanguagePref = useZefron((s) => s.setLanguagePref);
  const detectedLocation = useZefron((s) => s.detectedLocation);
  const detectedAt = useZefron((s) => s.detectedAt);
  const setDetectedLocation = useZefron((s) => s.setDetectedLocation);
  ...
  useEffect(() => {
    ...
    detectLocation(ctl.signal).then((loc) => { if (loc) setDetectedLocation(loc); }).catch(() => {});
    return () => ctl.abort();
  ...
  const effective = languagePref === "auto" ? detectedLocation?.languageName ?? "Global" : current.native;
            ...
            <div className="flex items-start gap-2 border-b border-border/60 bg-accent/40 px-3 py-2.5">
              ...
              <div className="min-w-0 text-xs">
                <p className="truncate font-medium text-foreground">
                  {detectedLocation.region ? `${detectedLocation.region}, ${detectedLocation.countryName}` : detectedLocation.countryName}
                </p>
                <p className="text-muted-foreground">Suggested: {detectedLocation.languageName}</p>
              </div>
            </div>
          )}
            ...
            {LANGUAGE_OPTIONS.map((opt) => {
              ...
              return (
                <li key={opt.code}>
                  <button
                    type="button"
                    onClick={() => { setLanguagePref(opt.code); setOpen(false); }}
                    className={cn(
