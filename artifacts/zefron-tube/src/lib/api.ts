...
const streamCache = new Map<string, Video>();
...
export async function searchVideos(
  ...
  for (const v of normalized) {
    metaCache.set(v.id, { title: v.title, channelName: v.channelName, thumbnail: v.thumbnail });
...
async function fetchBackupStream(
  ...
  try {
    ...
    return {
      ...
      thumbnail: data.videoId ? `https://i.ytimg.com/vi/${data.videoId}/hqdefault.jpg` : "",
...
export async function fetchStream(
  ...
  const enrich = (video: Video): Video => {
    ...
    let v = video;
    if (!v.title || v.title === "YouTube" || v.title.length < 3) v = { ...v, title: meta.title };
    if (!v.channelName || v.channelName === "YouTube" || /^unknown/i.test(v.channelName)) v = { ...v, channelName: meta.channelName };
  ...
  if (backup && backup.audioUrl) {
    const merged: ApiVideo = {
      ...
      duration: primaryVideo?.durationSeconds ? String(primaryVideo.durationSeconds) : backup.duration || "0",
  ...
  const results = await Promise.all(
    filtered.map(async ({ id, channel }) => {
      try {
        ...
        if (channel && (!v.channelName || v.channelName === "YouTube" || /^unknown/i.test(v.channelName))) {
