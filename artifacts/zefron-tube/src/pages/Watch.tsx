...
import { generateRelatedKeywords } from "@/lib/ai";
...
export function WatchPage() {
  ...
  const isLiked = useZefron((s) => s.isLiked);
  const playVideo = useZefron((s) => s.playVideo);

  useEffect(() => {
    if (!videoId) { setError("No video ID provided."); setLoadingVideo(false); return; }
    ...
    fetchStream(`https://youtu.be/${videoId}`, ctl.signal)
      .then((vid) => { setVideo(vid); setLoadingVideo(false); playVideo(vid); })
      .catch((e) => {
  ...
  useEffect(() => {
    ...
    if (cached) { setRelated(cached.filter((r) => !isBroken(r.id))); return; }
    let cancelled = false;
    setRelated([]);
    (async () => {
      try {
        const ai = await generateRelatedKeywords({ data: { title: video.title, channel: video.channelName } });
        const keywords = ai.keywords?.length ? ai.keywords : [video.title.split(/\s[-|–—]\s/)[0] || video.title];
        const all = await Promise.all(keywords.slice(0, 4).map((k) => searchAndResolve(k, 6).catch(() => [])));
        ...
        const pool = CATALOG.flatMap((s) => s.items).filter((it) => it.id !== video.id && !isBroken(it.id));
  ...
  useEffect(() => {
    ...
    window.addEventListener("zefron:videoended", playNext);
    window.addEventListener("zefron:videobroken", playNext);
    return () => {
      window.removeEventListener("zefron:videoended", playNext);
      window.removeEventListener("zefron:videobroken", playNext);
    };
  }, [related, playVideo]);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div id="player-slot" className="aspect-video w-full overflow-hidden rounded-2xl bg-black">
          ...
          {!loadingVideo && error && (
            <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-card text-center">
              ...
              <Link href="/" className="rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
                Back home
              </Link>
            </div>
          )}
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                onClick={() => toggleLike({
                  ...
                  thumbnail: video.thumbnail, youtubeUrl: video.youtubeUrl ?? "",
                  durationSeconds: video.durationSeconds,
                })}
                className={cn(
                  "inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-medium transition-colors",
                  liked ? "bg-primary text-primary-foreground border-primary" : "bg-surface text-foreground hover:bg-accent",
                )}
              >
      ...
      <aside className="space-y-3">
        <h2 className="px-1 font-display text-base font-semibold tracking-tight text-foreground">Related</h2>
        {related.length === 0 && (
        ...
        <div className="space-y-3">
          {related.filter((r) => !isBroken(r.id)).map((r) => (
            <Link
              key={r.id}
              href={`/watch?v=${encodeURIComponent(r.id)}`}
